...
namespace TradeCity.Engine.Resources
{
    ...
    public abstract class Resource : ITickable
    {
        ...
        {
            if (amount < 0) throw new ArgumentException("Can not gain negative amount");
            _stock += amount;
        ...
        {
            if (amount < 0) throw new ArgumentException("Can not spend negative amount");
            if (_stock >= amount)
            {
                _stock -= amount;
                OnAmountChange?.Invoke(-amount);
        ...
        {
            if (amount < 0) throw new ArgumentException("Can not set negative amount");
        ...
        {
            foreach (var res in from) Transfer(to, res, safe);
        }

        public static int GetStock(IEnumerable<Resource> list, Resource type)
        {
            return list.Where(res => res.GetType() == type.GetType()).Sum(res => res.GetStock());
        }
